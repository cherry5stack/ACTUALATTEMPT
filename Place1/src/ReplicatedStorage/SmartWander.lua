return function()
	...
	function SmartWander.findValidWanderPosition(enemyChar, maxAttempts, options, settings)
		...
		for attempt = 1, (maxAttempts or 10) do
			...
			-- ADD THIS: reject if floor is impassable for this NPC
			local floorMod = floorResult.Instance:FindFirstChildOfClass("PathfindingModifier")
			if floorMod and floorMod.Label and floorMod.Label ~= "" then
				local cost = settings and settings.AgentCosts and settings.AgentCosts[floorMod.Label]
				if cost == math.huge then
					table.insert(attemptsLog, string.format(
						"  Attempt %d: floor is impassable label='%s'", attempt, floorMod.Label
						))
					continue
				end
			end
