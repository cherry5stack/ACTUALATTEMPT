...
function DoorOpener.FindBlockingDoor(NPC, TargetPos, AgentInfo, overrideRange)
	...
	if ok and path.Status == Enum.PathStatus.Success then
		...
		for _, wp in ipairs(waypoints) do
			-- Only trust waypoints whose Label actually identifies a door link,
			-- not just proximity — prevents false positives from a door merely
			-- standing near an active fight the NPC has no need to route through.
			if wp.Label and string.find(string.lower(wp.Label), "door") then
				for _, doorModel in ipairs(CollectionService:GetTagged(DOOR_TAG)) do
					if doorModel:IsA("Model") and doorModel.Name == wp.Label then
